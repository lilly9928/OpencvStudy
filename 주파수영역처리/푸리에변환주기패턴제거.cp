...
Mat getFilter(Size size){
    ...
    for(int i = 0; i < tmp.rows ; i++){
        for(int j = 0; j <tmp.cols; j++){
            if(i>(tmp.rows/2 - 10)&& i < (tmp.rows/2 +10)&&j > (tmp.cols/2 +10))
                tmp.at<float>(i,j) = 0;
            else if (i > (tmp.rows/2 - 10)&& i < (tmp.rows/2 + 10)&& j < (tmp.cols/2 - 10))
                tmp.at<float>(i,j) = 0;
...
int main()
{
    Mat src =imread("test.jpeg",IMREAD_GRAYSCALE);
    Mat src_float,dft_images;
    imshow("original", src);
    
    src.convertTo(src_float, CV_32FC1,1.0/255.0);
    dft(src_float,dft_images,DFT_COMPLEX_OUTPUT);
    shuffleDFT(dft_images);
    displayDFT(dft_images);
    
    Mat lowpass = getFilter(dft_images.size());
    Mat result;

    multiply(dft_images, lowpass, result);
    displayDFT(result);

    Mat inverted_images;
    shuffleDFT(result);
    idft(result, inverted_images, DFT_SCALE| DFT_REAL_OUTPUT);
    imshow("test", inverted_images);
    waitKey(0);
    return 1;
